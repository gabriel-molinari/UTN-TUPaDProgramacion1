#TP Integrador Programacion - Gabriel Molinari - Fecha de entrega: 22/11/2025
...
#EN ORDEN: NOMBRE, POBLACION, SUPERFICIE, CONTINENTE
indice_nombre = 0
indice_poblacion = 1
indice_superficie = 2
indice_continente = 3
    ...
    with open(nombre_archivo, 'r', encoding='utf-8') as archivo:
        ...
        for linea in archivo:
            ...
            if len(campos) == 4:
                ...
                poblacion_str = campos[indice_poblacion].strip()
                superficie_str = campos[indice_superficie].strip()
...
def obtener_rango_valido(propiedad):
    ...
    minimo_str = input(f"INGRESE EL MÍNIMO DE {propiedad}: ")
    maximo_str = input(f"INGRESE EL MÁXIMO DE {propiedad}: ")

    # VALIDACIÓN DE QUE SON NÚMEROS ENTEROS
...
def ordenar_paises(paises_lista):
    ...
    paises_ordenados = sorted(paises_lista, key=operator.itemgetter(clave), reverse=reverso)
...
def mostrar_estadisticas(paises_lista):
    ...
    if paises_lista:
        ...
        pais_mayor_pop = max(paises_lista, key=operator.itemgetter('poblacion'))
        pais_menor_pop = min(paises_lista, key=operator.itemgetter('poblacion'))
